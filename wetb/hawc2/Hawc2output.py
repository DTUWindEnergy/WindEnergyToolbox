...
call ex.:
    ...
    file = view_sensors("HAWC2ex/test")
    # if called with ReadOnly = 1 as
    file = view_sensors("HAWC2ex/test",ReadOnly=1)
...
# Main read data call, read, save and sort data
    ...
    def __call__(self, ChVec=[], htc = None, name = None, desc = None, label = None):
        ...
        # and return all requested channels
            ...
            if self.ChInfo[0][0] == 'Time':
                channel_id = np.array(range(-1,self.NrCh-1))
...
if __name__ == '__main__':
    res_file = view_sensors('structure_wind')
