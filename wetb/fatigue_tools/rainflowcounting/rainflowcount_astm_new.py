...
Reference:
    ...
    West Conshohocken, PA: ASTM International, 2017, https://www.astm.org/e1049-85r17.html.
...
def count_cycles_new(series, ndigits=None, nbins=None, binsize=None):
    """Count cycles in the series.
    ...
    Returns
    -------
    MATLAB rainflow() style output:
    1. Cycle weights (1=Full cycle, 0.5= Halfcycle)
    2. Cycle ranges (peak to peak amplitude)
    3. Cycle mean
    4. Idx_start (corresponds to max/min)
    5. Idx_end (corresponds to min/max)
    """
    ...
    counts = []
    for rng, count, mean, i_start, i_end in cycles:        
        counts.append([count,rng,mean,i_start, i_end])
